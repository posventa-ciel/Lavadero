...
def main():
    ...
    for i, fila in enumerate(raw_data[1:], start=2):
        ...
        # Filtro de exclusión
        if any(x in pro_raw for x in ["NO SE LAVA", "SIN TURNO"]): continue
        ...
        # Si el asesor puso NO VINO, no se lava, pero lo guardamos para estadísticas de ausentes en Taller
        if "NO VINO" in pro_raw:
            pass # No se agrega a pendientes ni finalizados de lavadero
        elif estado != "FINALIZADO" or not tiene_fin_real:
        ...
        # 2. Eficiencia de Turnos (Taller)
        if es_de_fecha_filtro:
            hora_recep = fila[IDX_ING_DMS].strip()
            # LÓGICA: Se considera turno de Taller SOLO si tiene hora de recepción.
            if hora_recep != "":
                # El cliente NO VINO si en Horario Prometido dice explícitamente "NO VINO"
                ...
                with st.container():
                    ...
                    with r[8]:
                        ...
                        with c_txt:
                            ...
                            else: st.markdown(generar_badge_alertas(p['pro_dt'], now_dt, "FINALIZADO"), unsafe_allow_html=True)
                st.markdown("<div class='compact-row'></div>", unsafe_allow_html=True)
    ...
    with tab4:
        ...
        if turnos_eficiencia:
            ...
            ausentes_df = df_t[df_t['vino'] == False]
            vinieron_df = df_t[df_t['vino'] == True]
            
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Turnos Totales (Taller)", len(df_t))
            c2.metric("Vinieron", len(vinieron_df))
            c3.metric("No Vinieron", len(ausentes_df), delta_color="inverse")
            show_up = (len(vinieron_df)/len(df_t)*100) if len(df_t)>0 else 0
            c4.metric("Show-up Rate", f"{int(show_up)}%")
            ...
            with col_list1:
                ...
                if not ausentes_df.empty:
                    for _, a in ausentes_df.iterrows():
                        with st.container():
                            r = st.columns([1, 2, 1, 1])
                            r[0].write(f"**{a['dom']}**")
                            r[1].write(f"<small>{a['cli']}</small>", unsafe_allow_html=True)
                            r[2].write("Ausente")
                            if not a['rec'] and st.button("Recuperar", key=f"recu_{a['fila']}"):
                                hoja.update_cell(a['fila'], IDX_RECUPERO+1, "SI"); st.rerun()
                            elif a['rec']: st.write("✅")
                else: st.write("No hay ausentes registrados.")

            with col_list2:
                st.markdown("### 📋 Detalles de Turnos")
                st.write(f"DMS Programados: {len(df_t[df_t['dms']])}")
                st.write(f"Adicionales/Paracaídas: {len(df_t[~df_t['dms']])}")
                st.write(f"Servicios de Mantenimiento: {len(df_t[df_t['serv']])}")
