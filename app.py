...
# --- 4. FUNCIONES AUXILIARES (TUS ORIGINALES) ---
...
def main():
    ...
    for i, fila in enumerate(raw_data[1:], start=2):
        ...
        # ES FINALIZADO si dice Finalizado o si tiene la hora de fin de repaso (L)
        es_finalizado = (estado == "FINALIZADO") or (fila[IDX_FIN2] != "")
        
        # FILTRO DE FECHA (Para que no se mezclen días)
        ...
        if es_finalizado:
            ...
            if es_de_fecha: pendientes.append(item)
            ...
            for p in pendientes:
                with st.container():
                    ...
                    with c[4]:
                        ...
                        elif p['est'] == "REPASO": # ESTO TE FALTABA
                            if st.button("✅", key=f"fr{p['fila']}", type="primary"):
