            ...
            if not df_gastos.empty:
                ...
                if len(df_insumo) >= 2 and not df_autos.empty:
                    ...
                    if autos_c > 0:
                        ...
                        # --- TARJETAS DE INDICADORES (A TODO ANCHO) ---
                        st.markdown("<br>", unsafe_allow_html=True)
                        m1, m2, m3 = st.columns(3)
                        m1.metric("Rendimiento del Ciclo", f"{rend_x_auto:.3f} {unidad_ins}/auto")
                        m2.metric("Autos Lavados en Ciclo", f"{autos_c}")
                        m3.metric("Stock Actual Restante", f"{stock_actual} {unidad_ins}")
                        st.markdown("<br>", unsafe_allow_html=True)
                        ...
                        if historial_rend:
                            ...
                            fig_evol.add_trace(go.Bar(
                                ...
                                name='$/Auto', marker_color='#fbc02d', opacity=0.6, yaxis='y2'
                            ))
                            fig_evol.add_trace(go.Scatter(
                                x=df_hist['Ciclo'], y=df_hist['Rendimiento'], 
                                mode='lines+markers+text', name='Consumo', 
                                line=dict(color='#00235d', width=3), 
