...
def main():
    ...
    # ÍNDICES SEGÚN TU SHEET (A=0, B=1, ... I=8, J=9, K=10, L=11, M=12, N=13, O=14)
    ...
    for i, fila in enumerate(raw_data[1:], start=2):
        ...
        # Filtro de fecha: Solo lo que coincide con el calendario seleccionado
        es_de_fecha = (f_str in f_celda) or (f_str_cero in f_celda)
        if not es_de_fecha: continue # Saltamos todo lo que no sea de hoy

        # Clasificación: Solo FINALIZADO mueve abajo. PAUSA se queda arriba.
    ...
    with tab1:
        ...
        if pendientes:
            ...
            # Encabezados manuales para asegurar visibilidad
