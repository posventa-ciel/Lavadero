...
def procesar_prometido_completo(val, hoy, tz):
    ...
    if not val or ":" not in val: 
        return tz.localize(datetime(2099, 1, 1))
    
    try:
        if len(val) > 10: 
            # Formato completo con fecha: 04/02/2026 15:00
            dt = datetime.strptime(val, "%d/%m/%Y %H:%M")
        else:
            # Formato solo hora: 15:00 (Asume hoy)
...
def generar_badge_inteligente(prometido_dt, now_dt):
    # Comparar fechas de manera segura (ambas con timezone)
    es_hoy = prometido_dt.date() <= now_dt.date()
    hora_str = prometido_dt.strftime("%H:%M")
    
    if not es_hoy:
        return f"<div class='badge badge-blue'>{prometido_dt.strftime('%d/%m %H:%M')}<br>PRÓXIMO</div>"
    
    diff = (prometido_dt - now_dt).total_seconds() / 60
    
    if diff < 0: 
        return f"<div class='badge badge-red'>{hora_str}<br>DEMORADO</div>"
    elif diff <= 60: 
        return f"<div class='badge badge-red'>{hora_str}<br>YA!</div>"
    elif diff <= 120: 
        return f"<div class='badge badge-yellow'>{hora_str}<br>ATENCIÓN</div>"
    
    return f"<b>{hora_str}</b>"
...
def main():
    ...
    for i, fila in enumerate(raw_data[1:], start=2):
        ...
        # PROCESAR FECHA PROMETIDA CON ZONA HORARIA
        ...
        item = {
            ...
            "mod": fila[IDX_MOD], 
            "cli": fila[IDX_CLI], 
            "ase": limpiar_asesor(fila[IDX_ASE]),
            "pro_dt": dt_prometido, 
            "ini": fila[IDX_INI1], "fin": fila[IDX_FIN1],
            "ini2": fila[IDX_INI2], "fin2": fila[IDX_FIN2], 
            "est": estado, 
            "ok": fila[IDX_CTRL].strip().upper() in ["SI", "OK"], 
            "f_fin_real": f_fin_celda, 
            "trabajo": fila[IDX_TRABAJO].upper()
        }
        ...
        # Solo procesamos si la fecha coincide con el filtro
        if es_de_fecha:
            hora_b = fila[IDX_ING_DMS].strip()
            if hora_b != "":
                vino = not ("NO VINO" in fila[IDX_PRO].upper() or "NO VINO" in item['trabajo'])
                
                # Buscador ampliado de servicios
                ...
                turnos_eficiencia.append({
                    ...
                    "vino": vino, 
                    "serv": es_serv, 
                    "rec": (fila[IDX_RECUPERO].upper() == "SI")
                })
    ...
    with tab1:
        ...
        if pendientes:
            # Ordenamiento inteligente: Por fecha primero, luego hora
            pendientes.sort(key=lambda x: x["pro_dt"])
            
            cols_p = [0.8, 0.8, 1.4, 1.4, 0.8, 1.2]
            h_p = st.columns(cols_p)
            h_p[0].caption("ESTADO")
            h_p[1].caption("DOMINIO")
            h_p[2].caption("CLIENTE")
            h_p[3].caption("MODELO")
            h_p[4].caption("ASESOR")
            h_p[5].caption("ACCIONES")
            ...
            for p in pendientes:
                with st.container():
                    ...
                    # Badge Inteligente
                    badge_html = generar_badge_inteligente(p['pro_dt'], now_dt)
                        ...
                        if not p['ini']:
                            ...
                            with cb[1]:
                                if st.button("🏁", key=f"f{p['fila']}"):
                                    ...
                                    hoja.update_cell(p['fila'], IDX_FECHA_FIN+1, hoy_str)
                                    st.rerun()
        ...
        if finalizados_ver:
            ...
            h_f[2].caption("T.")
            h_f[3].caption("DOM")
            h_f[4].caption("CLIENTE")
            h_f[5].caption("MODELO")
            h_f[6].caption("ASESOR")
            h_f[7].caption("ESTADO")
                ...
                with st.container():
                    ...
                    with r[7]:
                        ...
                        with c_chk:
                            ...
                            if nk != t['ok']: 
                                hoja.update_cell(t['fila'], IDX_CTRL+1, "SI" if nk else "")
                                st.rerun()
                        with c_txt:
                            if t['ok']: 
    ...
    with tab2:
        ...
        if finalizados_ver:
            ...
            c3.metric("Máximo", f"{df_hoy['minutos'].max()} min")
            
            st.markdown("---")
            col_g1, col_g2 = st.columns(2)
            with col_g1:
                st.plotly_chart(px.bar(df_hoy, x='dom', y='minutos', color='minutos', title="Tiempo por Patente"), use_container_width=True)
            with col_g2:
                st.plotly_chart(px.pie(df_hoy, names='ase', title="Lavados por Asesor"), use_container_width=True)
        else:
            st.info("No hay autos finalizados hoy.")
    ...
    with tab3:
        ...
        for f in raw_data[1:]:
            if len(f) >= 12 and f[IDX_FECHA]:
                try:
                    ...
                    # Calculamos tiempo neto para todo el historial
        ...
        if hist_list:
            ...
            # --- HISTORIAL TALLER MENSUAL ---
            st.markdown("---")
            st.subheader(f"📊 Resumen Taller - {m_sel}")
            t_mes = []
            for f in raw_data[1:]:
                if len(f) >= 16 and m_sel in f[IDX_FECHA]:
                    try:
                        ...
                        if h_b != "":
                            ...
                            t_mes.append({
                                "Fecha": f[IDX_FECHA].split()[0], 
                                "DMS": (h_b != "13:00"), 
                                "Vino": v, 
                                "Rec": (f[IDX_RECUPERO].upper() == "SI")
                            })
                    except: continue
            
            if t_mes:
                df_tmes = pd.DataFrame(t_mes).groupby('Fecha').agg(
                    Turnos_DMS=('DMS','sum'), 
                    Asistencia=('Vino','sum'), 
                    Recuperados=('Rec','sum')
                ).reset_index()
    ...
    with tab4:
        ...
        if turnos_eficiencia:
            ...
            prog = df_t[df_t['dms'] == True]
            aus = prog[prog['vino'] == False]
            
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Turnos DMS", len(prog))
            asistencia_pct = int(len(prog[prog['vino']])/len(prog)*100) if len(prog)>0 else 0
            c2.metric("Show-up", f"{asistencia_pct}%")
            c3.metric("Adicionales", len(df_t[~df_t['dms']]))
            c4.metric("Servicios", len(df_t[df_t['serv']]))
            
            st.markdown("---")
            st.subheader("📞 Recupero de Ausentes")
            ...
            if not aus.empty:
                for _, a in aus.iterrows():
                    with st.container():
                        ...
                        r[2].write(f"<small>{a['mod']}</small>", unsafe_allow_html=True)
                        r[3].write(a['ase'])
                        
                        r[4].write("❌ PENDIENTE" if not a['rec'] else "✅ RECUPERADO")
                        
                        if not a['rec']: 
                            if r[5].button("Recuperar", key=f"rc_{a['fila']}"):
                                hoja.update_cell(a['fila'], IDX_RECUPERO+1, "SI")
                                st.rerun()
                    st.markdown("<div class='compact-row'></div>", unsafe_allow_html=True)
            else:
                st.success("No hay ausentes pendientes.")
            ...
            st.info("Sin turnos detectados hoy (Verificar Columna B).")
