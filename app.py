...
# --- 2. ESTILOS CSS (TU FORMATO ORIGINAL COMPACTO) ---
st.markdown("""
<style>
...
def main():
    ...
    st.markdown(f'<div class="header-box"><div class="header-title">PROGRAMACIÓN LAVADERO</div><div style="text-align: right;"><div style="font-size: 16px;">{fecha_sel.strftime("%d/%m/%Y")}</div><div>{hora_actual} hs</div></div></div>', unsafe_allow_html=True)
    ...
    for i, fila in enumerate(raw_data[1:], start=2):
        ...
        es_hoy = (f_str in f_celda) or (f_str_cero in f_celda)
        
        # Solo procesamos si es de hoy o si tiene un estado activo (está en el lavadero ahora)
        es_activo = estado in ["LAVANDO", "PAUSA", "REPASO"]
    ...
    with tab1:
        ...
        for t in terminados:
            with st.container():
                r = st.columns([0.6, 0.6, 0.8, 1.5, 0.8, 1.2])
                ...
                with r[5]:
                    chk = st.checkbox("", value=t['ok'], key=f"ck{t['fila']}", label_visibility="collapsed")
                    if chk != t['ok']:
                        hoja.update_cell(t['fila'], IDX_CTRL + 1, "OK" if chk else ""); st.rerun()
            st.markdown("<div class='compact-row'></div>", unsafe_allow_html=True)
