...
def main():
    ...
    for i, fila in enumerate(raw_data[1:], start=2):
        ...
        f_ingreso_raw = fila[IDX_FECHA].strip() # Tomamos la fecha de la primera columna
        f_fin_celda = fila[IDX_FECHA_FIN].strip()
        es_de_fecha_filtro = (f_str in f_ingreso_raw) or (f_str_cero in f_ingreso_raw)
        ...
        # Formateamos el ingreso para que se vea limpio (quitando el año)
        try:
            f_ing_dt = datetime.strptime(f_ingreso_raw.split()[0], "%d/%m/%Y")
            hora_ing = f_ingreso_raw.split()[1] if len(f_ingreso_raw.split()) > 1 else ""
            f_ing_display = f"{f_ing_dt.strftime('%d/%m')} {hora_ing}"
        except:
        ...
        if estado != "FINALIZADO" or not tiene_fin_real:
            if not busqueda or busqueda in dom: pendientes.append(item)
        else:
            if es_de_fecha_filtro or (fecha_sel == hoy_date and f_fin_celda == hoy_str):
                if not busqueda or busqueda in dom: finalizados_ver.append(item)
            
            # Historial
            try:
                if f_ingreso_raw:
                    f_h_dt = datetime.strptime(f_ingreso_raw.split()[0], "%d/%m/%Y")
                    t_n = calcular_tiempo_neto(item)
                    if t_n > 0: historial_global.append({"Fecha": f_h_dt, "Mes": f_h_dt.strftime("%Y-%m"), "Mins": t_n})
            except: pass

        # 2. Eficiencia de Turnos (CORREGIDA: Toma cualquier fila con Hora DMS o Hora Recepción)
        if es_de_fecha_filtro:
            hora_recep = fila[IDX_ING_DMS].strip()
            if hora_recep != "": # Lógica: Si tiene hora de recepción, es una unidad física en el taller
                vino = not ("NO VINO" in pro_raw or "NO VINO" in item['trabajo'])
                es_dms = (hora_recep != "13:00") # Asumimos que 13:00 son adicionales o sin turno DMS
                palabras_serv = ["SERV", "KM", "10K", "20K", "30K", "40K", "50K", "60K", "70K", "80K", "90K", "100K", "MANT"]
                es_serv = any(x in item['trabajo'] for x in palabras_serv)
                turnos_eficiencia.append({"fila":i, "dom":dom, "cli":fila[IDX_CLI], "mod":fila[IDX_MOD], "ase":item['ase'], "dms":es_dms, "vino":vino, "serv":es_serv, "rec":(fila[IDX_RECUPERO].upper() == "SI")})
    ...
    with tab1:
        ...
        if pendientes:
            ...
            # Col 0: Alerta, Col 1: Ingreso, Col 2: Dominio, Col 3: Cliente, Col 4: Modelo, Col 5: Asesor, Col 6: Botones
        ...
        if finalizados_ver:
            ...
            h_f = st.columns(cols_f); h_f[0].caption("HS. ENTREGA"); h_f[1].caption("INGRESO"); h_f[2].caption("INI"); h_f[3].caption("FIN"); h_f[4].caption("T."); h_f[5].caption("DOM"); h_f[6].caption("CLIENTE"); h_f[7].caption("ASESOR"); h_f[8].caption("ESTADO")
            for t in finalizados_ver:
                t['min_total'] = calcular_tiempo_neto(t)
                with st.container():
                    r = st.columns(cols_f)
                    r[0].markdown(f"<div style='text-align:center;'>{t['pro_str']}</div>", unsafe_allow_html=True)
    ...
    with tab4:
        ...
        if turnos_eficiencia:
            ...
            c1, c2, c3, c4 = st.columns(4); c1.metric("Unidades en Taller", len(df_t)); c2.metric("Show-up DMS", f"{int(len(prog[prog['vino']])/len(prog)*100)}%" if len(prog)>0 else "0%"); c3.metric("Adicionales", len(df_t[~df_t['dms']])); c4.metric("Servicios", len(df_t[df_t['serv']]))
            ...
            st.markdown("---")
            g1, g2 = st.columns(2)
            with g1: st.plotly_chart(px.pie(df_t, names='serv', title="Servicios vs Reparaciones", color_discrete_sequence=['#00235d', '#fbc02d']), use_container_width=True)
            with g2: 
                if not aus.empty: st.plotly_chart(px.bar(x=["Recuperados", "Pendientes"], y=[len(aus[aus['rec']]), len(aus[~aus['rec']])], title="Gestión de Recupero", color=["Rec", "Pen"], color_discrete_map={"Rec":"#2e7d32", "Pen":"#d32f2f"}), use_container_width=True)
